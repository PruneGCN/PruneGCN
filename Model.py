...
def subspace_hyperedge_contrast_loss(usr_mask, item_mask,  usr_embs, item_embs, Uancs, Ipos,  weight, threshold, temp,imp):
	...
	u_target_l = (u_mask @ u_mask.T) >= (u_threM_l - loosen_factor[imp]  )
	u_target_r = (u_mask @ u_mask.T) >= (u_threM_r - loosen_factor[imp]  )

	u_target = (u_target_l * u_target_r).float()
		

	i_target_l = (i_mask @ i_mask.T) >= (i_threM_l - loosen_factor[imp]  )  
	i_target_r = (i_mask @ i_mask.T) >= (i_threM_r - loosen_factor[imp]  )
...
def subspace_hyperedge_contrast_loss_v2(usr_mask, item_mask,  usr_embs, item_embs, Uancs, Ipos,  weight, threshold, temp,imp):
	if args.hyper_contr_resample:
		print("###################Uancs size#############################3")
		print(len(Uancs))
		print(len(Ipos))
	...
	ui_target_l = ui_mask_square >= (ui_threM_l - loosen_factor[imp]  )
	ui_target_r = ui_mask_square >= (ui_threM_r - loosen_factor[imp]  )
...
def subspace_kernel(embeds1_hid, embeds2_hid, mask1, mask2 ,nodes1,imp, temp=1.0):
	...
	target_l = masks_cross >= (threM_l - loosen_factor[imp]  )
	target_r = masks_cross >= (threM_r - loosen_factor[imp]  )
...
def subspace_hyperedge_contrast_loss_v3(usr_mask, item_mask,  usr_embs, item_embs, Uancs, Ipos,  weight, threshold, temp,imp):
	if args.hyper_contr_resample:
		print("###################Uancs size#############################")
		print(len(Uancs))
		print(len(Ipos))
	...
	loss = 0
	loss += subspace_kernel(usr_embs,  item_embs, usr_mask,  item_mask ,Uancs, imp, temp)
	loss += subspace_kernel(item_embs, usr_embs,  item_mask, usr_mask,  Ipos,  imp, temp)
	loss += subspace_kernel(usr_embs,  usr_embs,  usr_mask,  usr_mask,  Uancs, imp, temp)
	loss += subspace_kernel(item_embs, item_embs, item_mask, item_mask, Ipos,  imp, temp)
	loss *= weight
...
class Model(nn.Module):
	...
	def calcLoss(self, adj_tea, adj_stu, adj_closure,  ancs, poss, negs, opt,  imp_num = 0, train_flag=False, emb_spar=1.):
		...
		if (args.use_subspcreg) and (imp_num >= args.hyper_contr_start_imp) and (imp_num <= args.hyper_contr_end_imp) and (not args.train_middle_model):			
			...
			if args.subspcreg_version == 'v3':
				subsp_loss = subspace_hyperedge_contrast_loss_v3(usr_mask, item_mask,  suEmbeds_sub, siEmbeds_sub, uniqAncs, uniqPoss,  args.subspcreg, threshold, args.tempsubsp, imp_num)

			elif args.subspcreg_version == 'v2':
				subsp_loss = subspace_hyperedge_contrast_loss_v2(usr_mask, item_mask,  suEmbeds_sub, siEmbeds_sub, uniqAncs, uniqPoss,  args.subspcreg, threshold, args.tempsubsp, imp_num)
			else:
				#subsp_loss = subspace_hyperedge_contrast_loss(usr_mask, item_mask,  suEmbeds, siEmbeds, uniqAncs, uniqPoss,  args.subspcreg, threshold, args.tempsubsp, imp_num)
				subsp_loss = subspace_hyperedge_contrast_loss(usr_mask, item_mask,  suEmbeds_sub, siEmbeds_sub, uniqAncs, uniqPoss,  args.subspcreg, threshold, args.tempsubsp, imp_num)
		else:
