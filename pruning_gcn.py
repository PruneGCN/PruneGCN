...
def get_mask_distribution(Models, handler, args,   if_numpy=False, use_tea2drop_edges=False, use_mTea2drop_edges=True, use_adj_mask_aug=False):
    ...
    if use_adj_mask_aug:
        ...
        if args.use_SM_edgeW2aug:
            ...
            print("############args.use_SM_edgeW2aug#############, ", args.use_SM_edgeW2aug, type(args.use_SM_edgeW2aug))
...
def get_final_mask_epoch(Models, rewind_weight, args, handler,  keep_adj_weights2next_imp=False, use_adj_mask_aug=True, use_adj_random_pruning=False, use_emb_random_pruning=False, epoch=None, imp=None):
    ...
    adj_spar = adj_nonzero * 100 / model.adj_mask2_fixed.numel()
    ...
    return rewind_weight, adj_spar, wei_spar, emb_spar
...
def print_sparsity(model, torch_sparse=False, have_weight=False):
    ...
    print("-" * 100)
    print("Sparsity: Adj:[{:.2f}%] Wei:[ N/A %] Emb:[{:.2f}%]".format(adj_spar,emb_spar ))    
    print("-" * 100)

    wei_spar = -1.

    return adj_spar, wei_spar, emb_spar
